...
> **サイクル・多重リンクの挙動（design-review_2026-07.md R2-3）**
> - サイクル（循環リンク）を含むグループも `graphUtils.ts` の `buildTree` が visited-set で辿り済みのノードへの再訪問を防いでおり、無限ループにはならない。木構造化の際、サイクルを閉じる「戻りエッジ」は木構造には現れないが、リンクデータおよびCanvas上の表示（線）はそのまま残る。この挙動は許容し、特別なUI表現（警告等）は行わない。
> - 同じメモのペアに対する多重リンク（重複リンク）は作成時に弾く。`useLinks.ts` の `addLink` は a/b どちらの向きでも既存の isActive なリンクがあればそれを一本化して既存IDを返し、新規作成しない（Canvas・リスト双方のリンク作成経路が同じ `addLink` を通るため、抜け道はない）。
